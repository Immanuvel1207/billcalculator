...
function OrderDetails({ order, onClose, onUpdate }) {
  ...
  const markProductAsDone = async (productId) => {
    try {
      ...
      await axios.patch(`https://billcalculator-seven.vercel.app/api/orders/${order._id}/product/${productId}`, 
        { isDone: true },
  ...
  const updateOrderStatus = async (status) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`https://billcalculator-seven.vercel.app/api/orders/${order._id}`, 
        { status },
        { headers: { Authorization: `Bearer ${token}` }}
      );
